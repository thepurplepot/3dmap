...
options: struct {
    ...
    follow_camera_light: bool = false,
    ambient_light: f32 = 0.1,
...
pub fn update(self: *Self, window: *zglfw.Window) void {
    ...
    if (zgui.begin("Settings", .{ .flags = .{ .no_move = true, .no_resize = true, .no_title_bar = true } })) {
        ...
        if (zgui.checkbox("Flat Shading?", .{ .v = &self.options.flat_shading })) {
        }
        if (zgui.checkbox("Follow Camera Light?", .{ .v = &self.options.follow_camera_light })) {
        }
        if (zgui.sliderFloat("Ambient Strength", .{ .v = &self.options.ambient_light, .min = 0.0, .max = 1.0, .cfmt = "%.2f" })) {
        }
        if (zgui.sliderFloat("Specular Strength", .{ .v = &self.options.specular_strength, .min = 0.0, .max = 1.0, .cfmt = "%.2f" })) {
        }
    }
    zgui.end();
