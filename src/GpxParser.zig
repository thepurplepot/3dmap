...
fn endElement(ctx: ?*anyopaque, name: []const u8) anyerror!void {
    ...
    if (std.mem.eql(u8, name, "ele")) {
        data.in_tags = false;
        data.trkpts.items[data.trkpts.items.len - 1].ele = std.fmt.parseFloat(f32, data.data_buf.?) catch null;
