...
pub const XmlAttrIter = struct {
    ...
    pub fn reset(self: *XmlAttrIter) void {
        self.i = 0;
    }
};
...
pub const Callbacks = struct {
    ...
    charData: *const fn (ctx: ?*anyopaque, s: []const u8) anyerror!void,
};
...
fn charData(ctx: ?*anyopaque, s_c: [*c]const c.XML_Char, len: c_int) callconv(.C) void {
    const inner: *Inner = @ptrCast(@alignCast(ctx));
    const s = s_c[0..@intCast(len)];

    inner.callbacks.charData(inner.callbacks.ctx, s) catch |e| {
        _ = c.XML_StopParser(inner.parser, 0);
        inner.err = e;
    };
}